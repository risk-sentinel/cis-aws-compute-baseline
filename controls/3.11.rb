...
control 'C-3.11' do
  ...
  desc  'check', "
    ...
    1. Repeat steps 1-5 for each ECS cluster.
  ...
  desc  'fix', "
    ...
    1. Click `Save`.
    1. Repeat steps 1-8 for each ECS cluster requiring remediation.
  "
  tag severity:              'medium'
  tag nist:                  ['CM-8 a 1']
  tag cci:                   ['CCI-000389']
  ...
  offenders = aws_ecs_inventory.cluster_arns.each_with_object([]) do |arn, acc|
    ...
    if required.empty?
      acc << c.cluster_arn if c.tag_keys.empty?
    else
      missing = required - c.tag_keys
      acc << "#{c.cluster_arn}:missing=#{missing.join(',')}" unless missing.empty?
    end
  end

  describe 'ECS clusters missing tags (any-tag when required_ecs_tags is empty; listed keys otherwise)' do
    subject { offenders }
    it { should be_empty }
  end
end
